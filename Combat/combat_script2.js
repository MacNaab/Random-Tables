...
$("#sortable1").append('<li id="Ennemie'+Nbre+'" class="ui-state-default"><div class="grid-container">'+barre1+barre2+'</div><b><span onclick="change(this.id)" style="cursor: text" id="Ennemie_Nom_'+Nbre+'">'+Nom+'</span></b>, initiative : <span id="Ennemie_Ini_'+Nbre+'">'+Initiative+'</span>, PS: <span id="Ennemie_PV_'+Nbre+'">'+PV+'</span>/'+PV+', Endurance: <span id="Ennemie_STA_'+Nbre+'">'+Endurance+'</span>/'+Endurance+'<span id="Ennemie_Nombre_'+Nbre+'" style="display: none;">0</span></li>');
	...
	for(let i=1;i <= e;i++){
		...
		function ini_PNJ(){
			...
			var Rand = Math.floor(Math.random() * Math.floor(10))+1;
				if(Rand == '1'){var Rand2 = Math.floor(Math.random() * Math.floor(10))+1;var Rand = -Rand2;}
				if(Rand == '10'){var Rand2 = Math.floor(Math.random() * Math.floor(10))+1;var Rand = Number(Rand)+Number(Rand2);}
			var Initiative = Number(REF)+Number(Rand);
			return Initiative;		
		}
		var Initiative = ini_PNJ();
		var PV = found.Caract.PS;
		var Endurance = found.Caract.END;
		...
		$("#sortable2").append('<li id="Ennemie'+Nbre+'" class="ui-state-default"><div class="grid-container">'+barre1+barre2+'</div><b><span onclick="change(this.id)" style="cursor: text" id="Ennemie_Nom_'+Nbre+'">'+Nom+'</span></b>, initiative : <span id="Ennemie_Ini_'+Nbre+'">'+Initiative+'</span>, PS: <span id="Ennemie_PV_'+Nbre+'">'+PV+'</span>/'+PV+', Endurance: <span id="Ennemie_STA_'+Nbre+'">'+Endurance+'</span>/'+Endurance+'<span id="Ennemie_Nombre_'+Nbre+'" style="display: none;">0</span></li>');
		document.getElementById("Ennemie"+Nbre).style.paddingBottom = "25px";
	  	var option = document.createElement("option");
...
function Combat(){
    ...
    if(PV == null || PV == "" || PV == "0"){}
        ...
        if(PV_aff <= "0"){
			document.getElementById('Barre1_ennemie_'+nom).style.width = "0%";
...
function ATQ(){
	...
	var carac = Number(document.getElementById('ATQ_Caractéristique').value);
		if(carac == "1"){carac = found.Caract.INT;}
		else if(carac == "2"){carac = found.Caract.RÉF;if(carac==undefined){carac = found.Caract.REF;}}
		else if(carac == "3"){carac = found.Caract.DEX;}
		else if(carac == "4"){carac = found.Caract.COR;}
		else if(carac == "5"){carac = found.Caract.VIT;}
		else if(carac == "6"){carac = found.Caract.EMP;}
		else if(carac == "7"){carac = found.Caract.TECH;}
		else if(carac == "8"){carac = found.Caract.VOL;}
		else if(carac == "9"){carac = found.Caract.CHA;}
	var compt = document.getElementById('ATQ_Comptétence').value;
		if(found.Compt.hasOwnProperty(compt) == true){compt = found.Compt[compt];}else{compt=0;}
...
function DEF(){
	...
	var compt = document.getElementById('DEF_Comptétence').value;
		if(found.Compt.hasOwnProperty(compt) == true){compt = found.Compt[compt];}else{compt=0;}
